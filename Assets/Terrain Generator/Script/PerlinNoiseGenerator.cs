...
public class PerlinNoiseGenerator : MonoBehaviour
{
    ...
    // Optional compute shader. If assigned, generation runs on the GPU, otherwise CPU.
    ...
    /// <summary>
    /// Generates a 2D heightmap using multi-octave Perlin noise.
    /// If a compute shader is assigned, the work is dispatched to the GPU; otherwise the CPU path is used.
    /// </summary>
    public float[,] GeneratePerlinNoise() 
    {
        if (perlinCompute == null)
        {
            return GeneratePerlinNoiseCPU();
        }
